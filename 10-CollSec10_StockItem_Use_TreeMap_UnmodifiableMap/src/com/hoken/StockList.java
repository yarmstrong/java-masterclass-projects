...
public class StockList {
    ...
    public int addStock(StockItem item) {
        if (item != null) {
            /* ok gets. the logic why not just update the existing stock is bec the very same named item, can be
             freely given with a new price. the flaw here is that the main() method is not utilizing the getter for
             item price. during addStock, not only the quantity but also the price is being updated bec we are
             saving the item and not just updating the old one if its already in the stocklist. anyway thats how
             the main() wants to roll so okidoks */
